...
The 2 data sets used is publicly available from OECD website: cognitive item and student questionnaire data. Compendia and Codebook of PISA 2018 survey provide additional information to understand the 2 main data sets.
...
Using tabulate and graph builder, maximum number of questions corresponding to each Book ID confirms the presence of two clusters of science assessment (Figure 10). Book ID 13-24 have a total of 40 questions while the remainders have a total of 20 questions. 
...
The null hypothesis (H0) to be tested is that there is no difference in reading proficiency between gender. Oneway analysis is performed to confirm presence of an effect with p-value criteria of <0.05 for H¬0 rejection. The returned p-value is <0.05 which suggests difference in reading score between gender is statistically significant (Figure 22).
...
The returned p-value <0.05 indicates the score distribution is not normal for both gender groups (Figure 23). Next, variance equality for the two sample groups is checked (Figure 24). The small p-value in all 5 tests indicates unequal variances. Welch’s t-test returns small p-value which suggests significant score difference between gender. However, Welch’s t-test also assumes normal distribution.
...
As the assumption on normal distribution and homoscedasticity are violated, non-parametric test (Wilcoxon test) is used in place of t-test (Figure 25). Wilcoxon test returns p-value <0.05 which confirms the significance of gender in readings score.
...
According to Figure 19, male student in private school seems to perform better in mathematics than female student. T-test returns p-value of 0.0502 which suggests no statistically significant difference in mathematics score between gender.
