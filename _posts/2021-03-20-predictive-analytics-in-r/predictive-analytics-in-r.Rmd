...
```{r, echo=FALSE, fig.align='center', fig.cap='Components of tidymodels', class.source = 'fold-show'}
...
### Load R packages and pre-processed AirBnb data set
...
Character and logical variables are converted into factor variable. A quick summary of the dataset is provided using `skim` function from `skimr` package.
...
After we assess the correlation and feature importance, we might want to exclude several variables from our model. The removal can be done manually or we can use functions in `recipe` framework to do so. On top of feature selection, we also perform several processing steps as follows:
...
_All available steps in `recipe` can be found [here](https://recipes.tidymodels.org/reference/index.html)_

We will create 3 different recipes for linear regression, decision tree, and random forest model as they have different requirement of data transformation. For  the first linear regression, missing value has to be removed from the data set as they are not handled by the algorithm, whereas we can leave it for decision tree model.
...
rcpTransform3 <- recipe(target_var ~ ., data = listing_train) %>%
  update_role(id, new_role = "id variable") %>%
  step_naomit(all_predictors()) %>% #remove rows with NA
  step_other(all_predictors(), -all_numeric(),
             threshold = 0.05, other = "Others")
...
We can further evaluate those predicted values which deviate greatly from the actual values. First, we get the top 8 predictors from training model (based on p.value) and prepare the filtered training set.
...
top_err <- listing_pred %>%
  ...
  mutate(pred_error = abs(target_var-.pred)) %>%
  select(slp, id, pred_error) %>%
  gather(key, value, -id, -pred_error) %>%
  mutate(id = as.factor(id))
```

We can then plot the two data set with selected 6 predictors by:  
...
With regularised linear regression, we observe no significant improvement in RMSE score where it stays at 100.3 and we still see the same behaviour where the model underestimates the predicted price for listing price above \$250.
...
The subsequent steps are similar to that of GLM model. For the tree plotting, we use `fancyRpartPlot` function from `rattle` package. However, the visualisation becomes cluttered as the tree grows too deep.
...
rattle::fancyRpartPlot(final_tree_fit$fit, sub = "Decision Tree model")
...
final_metric_dtree
```
With decision tree, we get 91 RMSE score which is an improvement from the GLM model.
