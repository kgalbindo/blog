...
At this point of time, the difference between 2009 and 2019 can be seen between 20-24 and 30-44 age groups. To get the percentage for each age group, right click on the _SUM(2009)_ bullet, select _Quick Table Calculation_, and choose _Percent of Total_. Do the same for _SUM(2019)_. To make the comparison clearer, some adjustments are made:  
...
Duplicate the first worksheet and rename it to "Part 2". Next, remove the _Age group_ bullet from the _Rows_ shelf so that the distribution is grouped into just the 3 aggregated age groups. Right click on the _Age group (group)_ bullet and enable _Show Header_. Adjust the x-tick mark accordingly so that it shows the entire range of bar chart. The bar chart is also adjusted to be slightly thinner.
...
Hold Ctrl in the keyboard while clicking the continuous _Year_ bullet, drag it from _Columns_ shelf into the _Color_ pane to activate coloring on the line chart.
